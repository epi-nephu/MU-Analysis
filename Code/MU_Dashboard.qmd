...
mu_innermelb_sal.sf <- vic_sal_shp %>% 
  filter(sal_name %in% c("Ascot Vale", "Brunswick (Vic.)", "Brunswick West", "Coburg", "Pascoe Vale South", "Essendon", "Moonee Ponds", "Strathmore (Vic.)")) %>% 
  st_transform(crs=4326)
...
#datapt_31631 <- filter(mu_data_config, event_id=="320256131631")
```
...
```{r}
inclu_lga <- c("Moonee Valley (C)", "Moreland (C)", "Greater Geelong (C)", "Darebin (C)", "Yarra (C)", "Banyule (C)", "Boroondara (C)")
...
monthly_sequence <- seq(start_date, end_date, by = "month")

monthly_seq_df <- data.frame(epi_MY = monthly_sequence) %>% # code multiplies row by row n times
...
monthly_seq_df <- do.call(rbind, replicate(7, data.frame(epi_MY = monthly_sequence), simplify = FALSE)) %>% 
  ...
  mutate(lga = rep(inclu_lga, each=46)) # each = number of months in required period ie: 12mths x 3 yrs + 10mths
...
cumsum_data <- counts_mth_lga %>% 
  ...
  mutate(cum_count = cumsum(total), 
         lga = fct(lga, levels =c("Greater Geelong (C)", "Moreland (C)", "Moonee Valley (C)", "Boroondara (C)", "Banyule (C)", "Darebin (C)", "Yarra (C)")))
