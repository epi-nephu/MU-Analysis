...
Data period: Data from 1 January 2021 - 31 August 2025
...
mu_data_raw <- rio::import(here("Data", data_file)) %>% 
  clean_names() %>% 
  remove_empty("cols") %>% 
...
mu_data_config <- mu_raw %>% 
  ...
  # date when codes no longer used for lphu
  #filter(event_date >= "2022-07-01") %>% 
...
tabyl(mu_nephu_singleline, primary_exposure)

mu_nephu_prob <- filter(mu_nephu_singleline, defn=="Probable")

prob_ids <- mu_nephu_prob$event_id

mu_raw_nephu_prob <- filter(mu_data_config, event_id %in% prob_ids)
...
ggplot(data = mu_nephu_risk_counts) +
  ...
  #eom_line(aes(x= epi_MY, y=pct_nephu * sec_sf, group=1, col="percentage")) + 
...
**Monthly cases in NEPHU catchment from Jul 2022 - Jun 2025 and percentage of VIC cases**
...
**Monthly cases by LPHU from Jul 2022 - Jun 2025**
