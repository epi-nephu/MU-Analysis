...
mu_nephu_risk <- mu_nephu %>% 
  ...
  mutate(risk_factor = factor(risk_factor, levels=risk_factor_levels), 
         ...
         exp_lga = case_when(lga_2=="Unknown Victoria" ~ "Mornington Peninsula", # recode as names states Mornington Peninsula
                             lga_2=="Queenscliffe" ~ "Queenscliff", # correction of lga spelling
                             TRUE ~ exp_lga), 
...
cluster_cutoff_date <- max_date - months(cluster_cutoff_months)

cluster_data <- mu_data_config %>% 
  filter(lga %in% nephu_lgas) %>% 
  remove_empty("cols") %>% 
  distinct(event_id, .keep_all=TRUE) %>% 
  filter(event_date >= cluster_cutoff_date)
  

cluster_coords_mat <- as.matrix(cluster_data[, c("longitude", "latitude")])
...
cluster_data$cluster <- cluster_detect$cluster


# convert to sf object
cluster_data.sf <- st_as_sf(cluster_data, coords=c("longitude", "latitude"))
cluster_group.sf <- filter(cluster_data.sf, cluster>0)

label_cluster_resid <- sprintf(
  "ID: %s<br/>Date: %s<br/>Risk: %s<br/>Exposure LGA: %s<br/>Cluster Number: %s", 
  cluster_group.sf$event_id, cluster_group.sf$event_date, cluster_group.sf$risk_factor, cluster_group.sf$lga_2, cluster_group.sf$cluster
  ) %>% lapply(htmltools::HTML)

cluster_pal <- colorFactor(
  palette = c("red", "orange"), 
  domain = cluster_group.sf$cluster
)
