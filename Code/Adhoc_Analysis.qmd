...
mu_clean <- mu_raw %>% 
  clean_names() %>% 
  remove_empty("cols") %>% 
  select(-c(created_by, create_date, modified_by, modification_date, status, starts_with("enhanced_surveillance"), discharge_summary_requested, unid, event_id_2, status_2, event_id_3, event_id_4, party_id)) %>% 
...
epimonth_breaks <- c(seq.Date(from=min(mu_clean$epi_MY), to=max(mu_clean$epi_MY), by="month"), 
                           max(mu_clean$epi_MY) + months(1))
...
Determine the size of MU cases in Inner North Melbourne areas where it is considered endemic - Moonee Valley LGA (Ascot Vale, Essendon, Moonee Ponds, Strathmore) and Merri-bek LGA (Brunswick West, Coburg, Pascoe Vale South). Note that Ascot Vale and Coburg are latest additions. And compare it to NEPHU LGAs (Darebin, Yarra, Banyule, Boroondara)

Note in dataset, Moreland LGA (previous name) = Merri-bek LGA

## Explore the  case numbers in the 2 LGAs
...
mu_clean <- mu_data_config
mu_in_north <- mu_clean %>% 
...
ggplot(data = counts_mth_lga) +
  #geom_col(aes(x = epi_MY, y=total_nephu, group = 1, fill = "MU cases"), col="gray") + 
  geom_line(aes(x= epi_MY, y=total, group=1, col=lga)) + 
...
cumsum_plot <- ggplot(cumsum_data) + 
  ...
  scale_color_manual(values=c("green", "blue", "skyblue", "pink", "orange", "red", "yellow")) + 
  scale_x_date(expand=c(0,0), date_breaks="3 months", date_minor_breaks="month", date_labels="%b %Y", name="") +
...
ggsave(here("Output", "Cumsum_Plot.jpg"), plot=cumsum_plot)
...
```{r}
boroondara_2024 <- mu_clean %>% 
  filter(lga=="Boroondara (C)") %>% 
  filter(epiyear==2024)

western <- mu_clean %>% 
  filter(lga %in% c("Moonee Ponds (C)", "Moreland (C)"))

tabyl(western, risk_factor, epiyear)

tabyl(boroondara_2024, risk_factor)
...
Data period: Data from `r format(min_date, "%d %b %Y")` - `r format(max_date, "%d %b %Y")`
...
```{r}
cat("title=", paste0("Monthly cases by LPHU from ", format(min_date, '%b %Y'), " - ", format(max_date, '%b %Y')))
...
```{r}
cat("title=", paste0("Cumulative cases from ", format(min_date, "%b %Y"), " - ", format(max_date, "%b %Y")))
...
ggplot(data = mu_nephu_risk_counts) +
...
flextable(risk_table %>% select(-valid_percent)) %>% 
