...
# create polygon for inner melbourne endemic area - using POA
# mu_innermelb_poa.sf <- aus_poa_shp %>% 
#   # filter(as.numeric(POA_CODE21) %in% c(3039, 3040, 3041, 3044, 3055)) %>% 
#   filter(POA_CODE21 %in% c("3039", "3040", "3041", "3044", "3055")) %>% 
#   st_transform(crs=4326)
# 
# mu_new_innermelb_poa.sf <- aus_poa_shp %>% 
#   filter(POA_CODE21 %in% c("3058", "3032")) %>% 
#   st_transform(crs=4326)
...
#   #addCircles(data=cluster_group.sf, radius=5, opacity=0.8, fillOpacity=0.6, group="Cluster", color="red", 
#   #           label=label_cluster_resid) %>% 
#   addPolygons(data=mu_innermelb_sal.sf, color="red", label=label_innermelb) %>% 
#   addPolygons(data=mu_innermelb_poa.sf, color="blue", label=label_innermelb) %>% 
#   addPolygons(data=mu_new_innermelb_poa.sf, color="blue", dashArray="5, 10", label=label_innermelb) %>% 
...
map_cutoff_start_date <- max_date - months(cluster_cutoff_months)

map_data <- mu_data_config %>% 
  filter(lga %in% nephu_lgas) %>% 
  remove_empty("cols") %>% 
...
Map below shows both the residential and primary risk exposure locations of NEPHU *M. ulcerans* cases notified during the whole data period: `r format(min_date, '%b %Y')` - `r format(max_date, '%b %Y')`. This is an interactive map allowing you to zoom in and out and look at further details when hovering above the markers.  
...
leaflet() %>% 
  ...
  addPolygons(data=innermelb_new_sal.sf, color="red", dashArray="5, 10", label=label_innermelb) %>% 
  addPolygons(data=nephu_boundary_4326.sf, color="black", fill=FALSE) %>% 
  addLayersControl(overlayGroups=c("Residential", "Exposure", "Cluster"), 
...
```{r}
inclu_lga <- c("Moonee Valley", "Moreland", "Greater Geelong", "Darebin", "Yarra", "Banyule", "Boroondara")
...
# mu_west_barwon <- mu_clean %>% 
#   filter(lga %in% inclu_west_barwon)
...
monthly_seq_df <- do.call(rbind, replicate(7, data.frame(epi_MY = monthly_sequence), simplify = FALSE)) %>% 
...
cumsum_data <- counts_mth_lga %>% 
  ...
  mutate(cum_count = cumsum(total), 
         lga = fct(lga, levels =c("Greater Geelong", "Moreland", "Moonee Valley", "Boroondara", "Banyule", "Darebin", "Yarra")))

cumsum_plot <- ggplot(cumsum_data) + 
  geom_line(aes(x=epi_MY, y=cum_count, group=lga, col=lga), linewidth=1) + 
  scale_color_manual(values=c("orange", "red", "pink", "yellow", "green", "blue", "skyblue")) + 
  scale_x_date(expand=c(0,0), date_breaks="3 months", date_minor_breaks="month", date_labels="%b %Y", name="") +
  ...
  theme(plot.title = element_text(size=12), 
        ...
        axis.text.x = element_text(angle = 45, hjust=1)) + 
  labs(title="Cumulative case numbers by month", 
...
ggplot(counts_yr_lga) + 
  ...
  scale_fill_manual(values=c("orange", "red", "pink", "yellow", "green", "blue", "skyblue"), name="LGA") + 
